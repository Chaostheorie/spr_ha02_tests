...
class Test_Mkfile:
    ...
    def test_mkfile_easy(self):
        ...
        )  # fs.inodes[0] is the root node. its first direct block should point to the 1st inode (where the new file is located)
    ...
    def test_mkfile_nested(self):
        ...
        inode = mkdir(fs, "newDir", 1, 0, 0)
        inode.check(fs, check_parents=True)
        inode[0] = 2
        # craete file inode (virtual)
        file = TInode(fs, "newFile", NodeType.reg_file, 1, 2)

        # the actual test
        retval = libc.fs_mkfile(ctypes.byref(fs), ctypes.c_char_p(bytes(f"/{inode.name}/newFile", "UTF-8")))
        assert retval == 0
        file.check(fs)
        assert fs.inodes[2].n_type == 1  # meaning it is marked as regular file
        assert fs.inodes[2].name.decode("utf-8") == "newFile"
        assert (
            fs.inodes[1].direct_blocks[0] == 2
        )  # fs.inodes[1] is the node of /newDir. Its direct block [0] should point to the inode[2] (where the new file is located)
        assert fs.inodes[1].parent == 0
    ...
    def test_mkfile_wrong_name(self):
        fs = setup(5)
        retval = libc.fs_mkfile(
            ctypes.byref(fs), ctypes.c_char_p(bytes("testFile", "UTF-8"))
        )  # new file would be located at inode 1 if it wasn't for the error
        assert retval == -1
    ...
    def test_mkfile_complex(
        self,
    ):  # i assume when u get direct blocks right than u got the parent right as well cuz its basically the same lol
        fs = setup(9)
        retval = libc.fs_mkfile(ctypes.byref(fs), ctypes.c_char_p(bytes("testFile", "UTF-8")))
        assert retval == -1
        assert fs.inodes[0].direct_blocks[0] == -1
